...
export class LayeredMapWidgetComponent implements AfterViewInit, OnDestroy {
  ...
  leaf!: typeof L;
  allLayers: MyLayer[] = [];
  @ViewChild('mapContainer', { read: ElementRef, static: true })
  mapReference!: ElementRef<HTMLDivElement>;
  ...
  private onOverlayAdd(event: L.LayersControlEvent): void {
  ...
  private onOverlayRemove(event: L.LayersControlEvent): void {
    const layer = this.allLayers.find((l) => l.group === event.layer);
  ...
  private autoCenterIfNeeded() {
    if (this.cfg.autoCenter === 'true') {
  ...
  private onPositionUpdate(
    layers: MyLayer[],
    positionUpdates: PositionUpdateManagedObject[]
  ): void {
